---
name: refresh-docs
description: Use when the user asks to update / refresh / sync the project's README.md and CLAUDE.md files (top-level + per-module). Locates every doc with `find`, classifies each by path → concern, and rewrites stale sections so the docs match current code (handlers, schema, Make targets, design-doc pointers).
---

# Refresh project docs (README.md / CLAUDE.md)

This repo carries documentation at four levels:

| Level | Files | Concern |
|---|---|---|
| Repo root | `README.md`, `CLAUDE.md` | Project intro (humans) + agent-facing master index (cross-cutting standards). |
| Per-module | `modules/<svc>/README.md`, `modules/<svc>/CLAUDE.md` | Service intro + module-local agent guidance. `<svc>` ∈ `audit`, `auth`, `queue`, `shared`. |
| Sub-area | `deploy/k8s/README.md` | k8s layout walkthrough (cross-service composition). |
| Rules | `.claude/rules/*.md` | NOT updated by this skill — those are owned by humans (linked from CLAUDE.md). |
...
- **Don't duplicate** content between top-level and nested files. Top-level CLAUDE.md owns repo-wide conventions; per-module CLAUDE.md owns module-local facts. If the same statement appears in both, delete it from the nested file and link to the canonical one.
- **Never touch** files under `.claude/rules/`, `.claude/skills/`, or `.claude/agents/`. Those are governed separately.
- **Never touch** generated content: `infra/database/db/*.go`, `route/grpc/*.pb.go`, `atlas.sum`. README/CLAUDE files only describe these — they don't include their content.
...
```bash
find . -maxdepth 6 -type f \( -iname "README.md" -o -iname "CLAUDE.md" \) \
  -not -path "./node_modules/*" -not -path "./.git/*" -not -path "./.claude/*" | sort
```
...
- `./modules/audit/README.md`, `./modules/audit/CLAUDE.md`
- `./modules/auth/README.md`,  `./modules/auth/CLAUDE.md`
- `./modules/queue/README.md`, `./modules/queue/CLAUDE.md`
- `./modules/shared/README.md`,`./modules/shared/CLAUDE.md`
...
**Concern**: First-time visitor's orientation. NOT a substitute for CLAUDE.md.
...
**Keep it short**: what the project is, which services are inside, where to start (CLAUDE.md for agents, per-module READMEs for service-level details).

### `./CLAUDE.md` — agent-facing master index
...
The detailed rules live in `.claude/rules/coding-standards.md`, `testing.md`, `kubernetes-conventions.md` — keep CLAUDE.md as a high-level index that references them, not a duplicate.
...
- `.claude/rules/kubernetes-conventions.md` — but this README must complement, not duplicate, the rules file.
...
- リポジトリ規約 — ../../.claude/rules/coding-standards.md (+ testing.md, kubernetes-conventions.md)
- ルートの開発手順 — ../../CLAUDE.md
```

### `./modules/<svc>/CLAUDE.md` — module-local agent guidance

**Concern**: Things an agent needs that are TRUE FOR THIS MODULE ONLY. Don't repeat the top-level CLAUDE.md's cross-cutting rules.

**Required sections** (lean — under ~80 lines):

```markdown
# CLAUDE.md (modules/<svc>)

This file extends the repo-root CLAUDE.md with guidance specific to the `<svc>` module. The repo-root file remains the source of truth for cross-cutting conventions.
...
The shared module's CLAUDE.md is structurally different — see the existing file for the right shape (no design doc, no proto, no migrations).
...
grep -roE 'modules/[a-z]+/[^[:space:])"]+\.(go|sql|proto|yaml|md)' README.md CLAUDE.md modules/*/README.md modules/*/CLAUDE.md \
...
- **Authoring `.claude/rules/*.md`** — those are owned by humans; the skill `add-error-type` and others may touch specific rules but only their narrow scope.
...
|---|---|---|
| `README.md` | Humans landing on the repo | Was empty until 2026-05-07 — keep it short, point to CLAUDE.md and per-module READMEs. |
| `CLAUDE.md` | Agent master index | ~100 lines. Don't bloat — link to `.claude/rules/*.md`. |
...
| `modules/audit/CLAUDE.md` | audit module-local agent guidance | New 2026-05-07. |
| `modules/auth/README.md` | auth service intro | Was 5-line stub until 2026-05-07. |
| `modules/auth/CLAUDE.md` | auth module-local agent guidance | New 2026-05-07. |
| `modules/queue/README.md` | queue service intro | Most developed since 2026-05-07. Don't churn. |
| `modules/queue/CLAUDE.md` | queue module-local agent guidance | New 2026-05-07. |
| `modules/shared/README.md` | shared library intro | New 2026-05-07. |
| `modules/shared/CLAUDE.md` | shared module-local agent guidance | New 2026-05-07. |
